...
#Monkey patch for the error I keep getting
UNetSpatioTemporalConditionModel.__module__ = "diffusers.models.unets.unet_spatio_temporal_condition"
...
class DummyDataset(Dataset):
    def __init__(self, base_folder: str, num_samples=100000, width=1024, height=576, sample_frames=25):
        ...
        self.channels = 4 #For the extra grayscale channel
        self.width = width
        self.height = height
    ...
    def __getitem__(self, idx):
        ...
        canny_path = os.path.join(folder_path, "canny")

        frame_files = sorted([f for f in os.listdir(folder_path) if f.endswith(".jpg")])
        edge_files = sorted([f for f in os.listdir(canny_path) if f.endswith(".png")])

        if len(frame_files) < self.sample_frames or len(edge_files) < self.sample_frames:
            raise ValueError(f"Not enough frames in {chosen_folder}")

        start_idx = random.randint(0, len(frame_files) - self.sample_frames)
        selected_frame_files = frame_files[start_idx:start_idx + self.sample_frames]
        selected_edge_files = edge_files[start_idx:start_idx + self.sample_frames]

        pixel_values = torch.empty((self.sample_frames, self.channels, self.height, self.width))

        for i, (frame_file, edge_file) in enumerate(zip(selected_frame_files, selected_edge_files)):
            frame_path = os.path.join(folder_path, frame_file)
            edge_path = os.path.join(canny_path, edge_file)
